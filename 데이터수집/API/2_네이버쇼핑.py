...
print(f'가져온 상품 수 : {len(itemList)}')
print(itemList)


with open(f'./데이터수집/api/네이버 쇼핑' +
    f'{searchStr} {len(itemList)}건.json', 'w', encoding='utf-8') as f:
    #newsList를 json파일 포맷으로 변경
    jsonData = json.dumps(itemList, indent=4, sort_keys=False,
        ensure_ascii=False)
    #indent : 4칸 띄워서 저장
