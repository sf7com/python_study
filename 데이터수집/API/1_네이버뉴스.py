...
for start in range(1, 1000, 100) :
    ...
    for item in jsonResult['items'] :
        ...
        pDate = datetime.datetime.strptime(item['pubDate'],
            '%a, %d %b %Y %H:%M:%S +0900')
...
with open(f'./데이터수집/api/네이버 뉴스' +
    f'{searchStr} {len(newsList)}건.json', 'w', encoding='utf-8') as f:
    #newsList를 json파일 포맷으로 변경
    jsonData = json.dumps(newsList, indent=4, sort_keys=False,
        ensure_ascii=False)
    #indent : 4칸 띄워서 저장
    #sort_key : 키값 별 정렬, 정렬을 원하면 True
