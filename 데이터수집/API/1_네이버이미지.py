...
def getNaverImage(searchStr, start, display,filter='small'):
...
for start in range(1,1000,100) :
    jsonResult = getNaverImage(searchStr,start,100)
...
for idx,item in enumerate(itemList) :
    ...
    #파일의 확장자 정보 찾기
    m = re.search(r'\.(\w+?)$', link) #non-greedy : "?"" 최소한으로 가져오기
    fileExtention = "jpg" if m==None else m.group(1)
    fileName += "."+fileExtention
    

#링크정보로 데이터 다운로드시 dload
#pip install dload
    try :
