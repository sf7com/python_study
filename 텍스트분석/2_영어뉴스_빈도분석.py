...
#(1) 데이터 가져오기
data_df = pd.read_csv('./텍스트분석/data/abcNewsData.csv')
data_df.head()
data_df.info()
#publish_date 데이터 타입 int 
#headline_text 문자열 

#(2) 데이터 전처리
#(2-1) 정제 : 불필요한 기호나 문자 제거
data_df['전처리_제목'] = data_df['headline_text'].str.replace(r'[^a-zA-Z]+', " ")
data_df['전처리_제목']
...
data_df.head()
data_df['년도'] = data_df['publish_date'].astype('str').str[:4]
data_df.head()
sorted(data_df['년도'].unique(), reverse=True) # 2017~2003

#데이터 프레임 특정 칼럼 기준 정렬
data_df_sorted = data_df.sort_values(by=['년도'])
data_df_sorted['년도'].head() #내림차순 정렬

#group by를 통해 년도별 논문수 집계
group_df = data_df_sorted.groupby('년도', as_index=False)['headline_text'].count()
group_df
...
plt.grid(True)
plt.plot(group_df['년도'], group_df['headline_text'])
plt.show()
...
plt.imshow(cloud)
plt.axis('off') #축 제거
plt.savefig('./텍스트분석/영어 뉴스기사 시각화.jpg')
plt.show()
