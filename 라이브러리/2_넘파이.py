...
arr = np.ones(10) #1값으로 인수 크기만큼 채워준다.
arr
arr = np.ones((3,5)) #1값으로 인수 크기만큼 채워준다.
arr

arr = np.arange(10) #0~9까지 1씩 증가해서 array을 만듬
arr
arr = np.arange(3,10) #3~9까지 1씩 증가해서 array을 만듬
arr
arr = np.arange(3,10,0.1) #3~9까지 0.1씩 증가해서 array을 만듬
...
arr #5이상 10미만의 랜덤한 수 size 크기만큼의 array 만든다.
arr = np.random.randint(1, 10, size=(3,3))
arr #1이상 10미만의 랜덤한 수 size 크기만큼의 array 만든다.

#(3) Array의 인덱싱 (리스트의 인덱스 참조 하는 방식과 유사)
arr = np.arange(10)
arr[0]
arr[1:3] #인덱스 1이상 3미만
arr[::2] #처음부터 끝까지 인덱스 2씩 증가
arr[::-1] #거꾸로 가져오기
...
arr1+10 #arr1의 모든 요소에 10이 더해짐
arr1*2  #arr1의 모든 요소에 2가 곱해짐
arr1/3 #arr1의 모든 요소에 3이 나눠짐
...
arr1 = np.array([[1,2,3], #사칙연산 [10,11,12]
                [4,5,6]]) #사칙연산 [10,11,12]
arr3 = np.array([10,11,12])   #[[1,2,3]
                #[10,11,12])  #[4,5,6]
arr3+arr1 
arr1-arr3
arr1*arr3
...
#(5) Array 인덱싱 마스크
#내가 원하는 데이터만 얻고싶을때 사용
#비교연산자(==, >, <, 등) 마스킹값(True/False) 얻고
#마스킹 값이 True 데이터를 얻는다.
...
np.max(arr1) #모든 요소의 최댓값
np.max(arr1, axis=1) #열간의 최댓값
np.max(arr1, axis=0) #행간의 최댓값
#max를 min으로 바꾸면 최솟값도 얻을 수 있다.

#정렬
arr1 = np.array([[10,2,14],
                [3,4,1]])
...
arr1 = np.array([[10,2,14],
                [3,4,1]])
arr1 = np.sort(arr1, axis=0)[::-1, :] #행간 내림차순
arr1