...
def testFunc2(numList):
    numList.append(10) # 주소를 통해 값을 호출해서 값에 영향을 줌
    numList[0]= -100 # 주소를 통해 값을 호출해서 값에 영향을 줌
    numList = [1,2,3] # 새로운 객체로 주소값으로 리스트 생성해서 외부 값에 영향을 안 준다.
...
print(fibo(5))
print(fibo(6))
# 숫자가 커질 수록 메모리에 함수가 쌓여 계산하는데 문제가 생길 수 있다.