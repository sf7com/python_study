#자료구조 (Collection)
#데이터를 관리하고 저장하는 방식
#대용량일수록 메모리에 빨리 저장하고 빠르게 검색하여
#효율적으로 사용하고 실행시간을 줄인다.
...
numList.append(10)
numList.append(2)
print(numList.pop(0)) # 1 출력
print(numList) #1가 없어짐
print(numList.pop(0)) # 2 출력
...
'addr' in person2
#값 검색
'임꺽정' in person2.values()
'홍길동' in person2.values()
...
#인덱스 슬라이싱
for num in numList[::2] : #인덱스 1부터 끝까지 가져오기
    print(num)
...
for key in person.keys() :
    #get() 킷값이 없으면 None으로 리턴,
    #default값을 설정 가능
    print(key, person.get(key)) 
    #person[key] 킷값이 없으면 예외발생
...
fruitDic = dict()
for val in fruitSet :
    fruitDic[val] = fruits.count(val)
print(fruitDic)

#Collections 모듈 활용 과일 개수 구하기
#collectons 패키지에서 Counter 클래스 불러오기
